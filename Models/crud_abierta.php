...
ida_numrenglon) = :idsec");
    ...
    public function consultaGraficaCumplimientoCatalgo($usuario,$servicio,$nsec,$estandar,$mesinicial,$mesfinal){
...
`ida_numcaracteristica1`,'.',`ida_numcaracteristica2`,'.',`ida_numcaracteristica3`)=:seccion 
        ...
        AND `usuario`=:usuario    and  STR_TO_DATE(mes_asignacion,'%Y-%m-%d') >:mes_inicial 
 and  STR_TO_DATE(mes_asignacion,'%Y-%m-%d') <=:mes_final 
       ...
       // die($nsec);
        ...
        $stmt-> bindParam(":seccion", $nsec, PDO::PARAM_STR);
         $stmt-> bindParam(":mes_inicial", $mesinicial, PDO::PARAM_STR);

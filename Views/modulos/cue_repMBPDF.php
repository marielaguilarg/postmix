...
//$pdf->Rect(160,243,48,5);
//$pdf->Rect(15,253,193,13);
...
$pdf->Cell(0,6,'ANALISIS MICROBIOLOGICO', 0, 0 ,'C', true);

$respuesta = DatosMuestra::vistaItem($ntoma, "MB", "aa_muestras");
...
//    $pdf->Text(10,64,$ntoma);
	...
	$pdf->SetY(61);
    $pdf->SetX(33);
	$pdf->Cell(20,4,'MICROBIOLOGICO', 0, 0 ,'C', FALSE);
...
//	$pdf->Cell(30,4,$fecmue, 0, 0 ,'C', FALSE);
    ...
    $resul = DatosMuestra::vistaResultados($idserv, 1, 'MB', $tipomue, "aa_pruebaanalisis");
    ...
    foreach ($resul as $key => $rowd) {
	   if (($i % 2)>0) {
		  ...
		  $pdf->SetX(22);
		  $pdf->SetFont('Arial','B',7);
		  $pdf->MultiCell(28,4,$rowd["red_parametroesp"], 0, '' , FALSE);

	      $pdf->SetY($Y);
		  $pdf->SetX(57);
		  $pdf->SetFont('Arial','B',7);
		  $pdf->MultiCell(24,4,$rowd["red_estandar"], 0, '' , FALSE);
	   ...
	   $pdf->SetY($Y1);
		  ...
		  $pdf->SetX(151);
		  $pdf->SetFont('Arial','B',7);
		  $pdf->MultiCell(25,4,$rowd["red_estandar"], 0, '' , FALSE);
			$Y1+=12;
			$YC1+=12;
	   }
	   $i+=1;
